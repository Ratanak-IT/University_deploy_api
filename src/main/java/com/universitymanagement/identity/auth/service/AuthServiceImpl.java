...
public class AuthServiceImpl implements AuthService {
    ...
    @Override
    public String buildLoginUrl(String state, String codeChallenge, String redirectUri) {
        return keycloakTokenService.buildAuthorizationUrl(state, codeChallenge, redirectUri);
    }

    @Override
    public LoginResponse exchangeAuthorizationCode(String code, String codeVerifier, String redirectUri) {
        LoginResponse response = keycloakTokenService.exchangeAuthorizationCode(code, codeVerifier, redirectUri);

        // Authorization Code flow មិនផ្ដល់ email ជា input ទេ ដូច្នេះយើងអាន email ចេញពី access token
        String email = extractEmailFromToken(response.getAccessToken());
        if (email != null) {
            recordSession(email, response);
        }
        return response;
    }
    ...
    private String extractEmailFromToken(String accessToken) {
        ...
        try {
            ...
            log.warn("Could not extract email from access token: {}", e.getMessage());
            return null;
        }
    }
