...
public class KeycloakTokenServiceImpl implements KeycloakTokenService{
    ...
    private static final String AUTHORIZE_ENDPOINT = "/realms/%s/protocol/openid-connect/auth";
    ...
    public LoginResponse login(String email, String password) {
        ...
        KeyCloakTokenResponse tokenResponse = client.post()
                .uri(TOKEN_ENDPOINT, properties.getTargetRealm())
    ...
    /**
     * បង្កើត URL ទៅ Keycloak login form (Authorization Code + PKCE flow).
     * Browser នឹង redirect ទៅ URL នេះ ដើម្បីឲ្យ user login នៅលើ form របស់ Keycloak ផ្ទាល់។
     */
    @Override
    public String buildAuthorizationUrl(String state, String codeChallenge, String redirectUri) {
        String base = properties.getServerUrl()
                + String.format(AUTHORIZE_ENDPOINT, properties.getTargetRealm());
    ...
    public LoginResponse exchangeAuthorizationCode(String code, String codeVerifier, String redirectUri) {
        ...
        KeyCloakTokenResponse  tokenResponse  = client.post()
                ...
                .onStatus(HttpStatusCode::is4xxClientError, (req , res) ->  {
                    String body = new String(res.getBody().readAllBytes(), StandardCharsets.UTF_8);
                    log.error("Keycloak code exchange error: status={} body={}", res.getStatusCode(), body);
                    throw new InvalidAuthorizationCodeException("Failed to exchange authorization code: " + body);
                })
                .onStatus(HttpStatusCode::is5xxServerError , (req , res) -> {
                    throw new KeycloakUnavailableException("Keycloak server error");
                })
    ...
    public void logout(LogoutRequest request) {
        ...
        client.post()
                .uri(LOGOUT_ENDPOINT, properties.getTargetRealm())
                .contentType(MediaType.APPLICATION_FORM_URLENCODED)
                .body(form)
                .retrieve()
                .onStatus(HttpStatusCode::is4xxClientError, (req, res) -> {
